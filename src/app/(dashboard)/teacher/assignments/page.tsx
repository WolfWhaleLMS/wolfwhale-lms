...
export default function TeacherAssignmentsPage() {
  ...
  const getUngradedCount = (assignment: MockTeacherAssignment) => {
  ...
  const totalAssignments = mockTeacherAssignments.length;
  const publishedCount = mockTeacherAssignments.filter((a) => a.status === 'published').length;
  const draftCount = mockTeacherAssignments.filter((a) => a.status === 'draft').length;
  const needsGradingCount = mockTeacherAssignments.filter(
    (a) => a.status === 'published' && getUngradedCount(a) > 0
  ).length;
  const totalUngraded = mockTeacherAssignments.reduce(
    (sum, a) => sum + (a.status === 'published' ? getUngradedCount(a) : 0),
    0
  );

  const getTypeColor = (type: MockTeacherAssignment['type']) => {
    switch (type) {
      case 'homework':
  ...
  const getStatusBadge = (assignment: MockTeacherAssignment) => {
