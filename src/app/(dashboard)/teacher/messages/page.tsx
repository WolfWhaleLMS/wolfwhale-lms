...
export default async function TeacherMessagesPage() {
  ...
  // Fetch conversations where the teacher is a member
  ...
  // Fetch unread count per conversation (messages not read by current user)
  ...
  // Fetch all messages in conversations to check unread
  ...
  const conversationData = conversationList.map((conv) => {
    ...
    const otherRole = otherMembers.length > 0 ? (roleMap[otherMembers[0]] || '') : '';
