...
import type { StudentAchievement, AchievementCategory, XPTransaction } from '@/types/gamification.types';
...
function computeLevelData(totalXP: number) {
  let levelIndex = 0;
  ...
  const nextLevelThreshold = LEVEL_THRESHOLDS[levelIndex + 1] || currentLevelThreshold + 500;
  const xpInLevel = totalXP - currentLevelThreshold;
  const xpNeededForLevel = nextLevelThreshold - currentLevelThreshold;
  const tierIndex = Math.min(Math.floor(levelIndex / 6), TIER_NAMES.length - 1);
...
function getStreakCalendar(streakDays: number): { date: string; active: boolean }[] {
...
export function AchievementsPageClient({ gamificationData }: AchievementsPageClientProps) {
  const [selectedCategory, setSelectedCategory] = useState<AchievementCategory | 'All'>('All');

  const xpState = gamificationData.xp || {
    totalXP: 0,
    currentLevel: 1,
    currentTier: 'Novice',
    streakDays: 0,
  ...
  const levelData = computeLevelData(xpState.totalXP);

  // Build earned achievements map from real data
  const earnedMap = useMemo(() => {
    const map = new Map<string, StudentAchievement>();
    gamificationData.achievements.forEach((sa) => {
      map.set(sa.achievementId, {
        achievementId: sa.achievementId,
        studentId: '',
        unlockedAt: sa.unlockedAt,
        displayed: sa.displayed,
      });
    });
    return map;
  }, [gamificationData.achievements]);

  // Map XP transactions to the XPTransaction type
  const xpTransactions: XPTransaction[] = useMemo(() => {
    return gamificationData.xpTransactions.map((t) => ({
      id: t.id,
      studentId: '',
      amount: t.amount,
      sourceType: t.sourceType,
      sourceDescription: t.description || t.sourceType,
      createdAt: t.createdAt,
    }));
  }, [gamificationData.xpTransactions]);

  const allAchievements = useMemo(() => getAllAchievements(), []);
  const streakCalendar = useMemo(
    () => getStreakCalendar(xpState.streakDays),
    [xpState.streakDays]
  );
  ...
  const tierName = TIER_NAMES[levelData.tierIndex] || 'Pup Recruit';
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-purple-50/30 to-amber-50/20 dark:from-slate-950 dark:via-purple-950/20 dark:to-amber-950/10 p-4 md:p-8">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          >
            <LevelBadge
              level={levelData.displayLevel}
              tierIndex={levelData.tierIndex}
              size="lg"
              showTierName={true}
              recentLevelUp={false}
            />
            ...
            {/* Daily/Weekly XP caps (shown as placeholders since we don't track these precisely) */}
            <div className="mt-4 w-full space-y-2">
              <div className="flex items-center justify-between text-xs">
                <span className="text-slate-500 dark:text-slate-400">Daily Cap</span>
                <span className="font-semibold text-slate-700 dark:text-slate-200">
                  -- / {GAMIFICATION_LIMITS.daily_xp_cap} XP
                </span>
              </div>
              <div className="h-1.5 bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden">
                <div className="h-full bg-gradient-to-r from-green-400 to-emerald-500 rounded-full w-0" />
              </div>
              <div className="flex items-center justify-between text-xs">
                <span className="text-slate-500 dark:text-slate-400">Weekly Cap</span>
                <span className="font-semibold text-slate-700 dark:text-slate-200">
                  -- / {GAMIFICATION_LIMITS.weekly_xp_cap} XP
                </span>
              </div>
              <div className="h-1.5 bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden">
                <div className="h-full bg-gradient-to-r from-blue-400 to-indigo-500 rounded-full w-0" />
              </div>
            </div>
          </motion.div>
          ...
          >
            <XPProgressBar
              ...
              xpNeeded={levelData.xpForCurrentLevel}
              level={levelData.displayLevel}
              tierIndex={levelData.tierIndex}
        ...
        >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 mb-6">
            {categoryStats.map((stat, i) => {
              const pct =
                stat.total > 0 ? Math.round((stat.earned / stat.total) * 100) : 0;
              return (
                <motion.button
                  key={stat.category}
                  className={cn(
                  ...
                  onClick={() =>
                    setSelectedCategory(
                      selectedCategory === stat.category
                        ? 'All'
                        : (stat.category as AchievementCategory)
                    )
                  }
                >
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                {STREAK_MILESTONES.map((milestone) => {
                  const achieved = xpState.streakDays >= milestone;
                  const progress = achieved
                    ? 100
                    : Math.round((xpState.streakDays / milestone) * 100);
                  return (
        ...
        >
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-2">
            {TIER_NAMES.map((tier, i) => {
              const isCurrentTier = i === levelData.tierIndex;
              const isAchieved = i <= levelData.tierIndex;
