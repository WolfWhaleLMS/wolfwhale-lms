...
export function AssignmentsPageClient({ assignments }: AssignmentsPageClientProps) {
  ...
  const filteredAssignments = useMemo(() => {
    let result = assignments.filter((a) => matchesFilter(a, activeFilter));
  ...
  }, [assignments, activeFilter, searchQuery, sortField, sortOrder]);
  ...
  const filterCounts = useMemo(() => {
    return FILTER_TABS.reduce(
      (acc, tab) => {
        acc[tab.value] = assignments.filter((a) => matchesFilter(a, tab.value)).length;
        return acc;
      },
