...
// ---------------------------------------------------------------------------
// STUDENT: Textbook Flashcard Decks (grouped by chapter)
// ---------------------------------------------------------------------------

export async function getTextbookFlashcardDecks() {
  const { supabase, user, tenantId } = await getContext()

  // Get student's assigned textbooks
  ...
  const textbookIds = (assignments || []).map((a) => a.textbook_id)
  if (textbookIds.length === 0) return []

  // Get chapters from assigned textbooks that have flashcards
  const { data: chapters } = await supabase
    .from('textbook_chapters')
    .select('id, textbook_id, title, chapter_number, is_published')
  ...
  // Get flashcard counts per chapter
  ...
  // Build chapter_id -> flashcard count map
  const flashcardCountMap: Record<string, number> = {}
  for (const f of flashcards || []) {
    flashcardCountMap[f.chapter_id] = (flashcardCountMap[f.chapter_id] || 0) + 1
  }

  // Only keep chapters that actually have flashcards
  const chaptersWithFlashcards = chapters.filter(
    (c) => (flashcardCountMap[c.id] || 0) > 0
  )

  if (chaptersWithFlashcards.length === 0) return []

  // Get textbook titles
  const uniqueTextbookIds = [...new Set(chaptersWithFlashcards.map((c) => c.textbook_id))]
  const { data: textbooks } = await supabase
    .from('textbooks')
    .select('id, title')
    .in('id', uniqueTextbookIds)

  const textbookNameMap: Record<string, string> = {}
  for (const tb of textbooks || []) {
    textbookNameMap[tb.id] = tb.title
  }

  // Get student progress for these flashcards
  const allFlashcardIds = (flashcards || [])
    .filter((f) => flashcardCountMap[f.chapter_id])
    .map((f) => f.id)
  ...
  if (allFlashcardIds.length > 0) {
    const { data: progress } = await supabase
      ...
      .select('flashcard_id, chapter_id, next_review_at')
      .eq('student_id', user.id)
      .eq('tenant_id', tenantId)
    ...
    const now = new Date()
    for (const p of progress || []) {
      if (!progressMap[p.chapter_id]) progressMap[p.chapter_id] = { total: 0, due: 0 }
      progressMap[p.chapter_id].total++
      if (!p.next_review_at || new Date(p.next_review_at) <= now) {
        progressMap[p.chapter_id].due++
      }
    }
  }

  return chaptersWithFlashcards.map((ch) => ({
    chapterId: ch.id,
    textbookId: ch.textbook_id,
    textbookTitle: textbookNameMap[ch.textbook_id] || 'Textbook',
    chapterTitle: ch.title,
    chapterNumber: ch.chapter_number,
    cardCount: flashcardCountMap[ch.id] || 0,
    studiedCards: progressMap[ch.id]?.total || 0,
    dueCards: progressMap[ch.id]?.due || 0,
  }))
}
