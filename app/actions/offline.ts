...
// ---------------------------------------------------------------------------
// downloadUserFlashcards — decks and cards for given courses
// ---------------------------------------------------------------------------

export async function downloadUserFlashcards(
  courseIds: string[]
): Promise<
  ActionResult<{
    decks: {
...
> {
  try {
    ...
    if (parsed.data.length === 0) {
      return { success: true, data: { decks: [], cards: [] } }
    }
    ...
    // Fetch published decks for the enrolled courses
    const { data: decks, error: deckError } = await supabase
      .from('flashcard_decks')
      .select('id, course_id, title, description, card_count')
      .eq('tenant_id', tenantId)
      .eq('status', 'published')
      .in('course_id', parsed.data)
    ...
    if (deckIds.length > 0) {
      const { data: cardData, error: cardError } = await supabase
        .from('flashcards')
        .select('id, deck_id, front_text, back_text, hint, order_index')
        .in('deck_id', deckIds)
        .order('order_index', { ascending: true })
    ...
    return {
      success: true,
      data: { decks: decks || [], cards },
    }
  } catch (err) {
    console.error('[offline] downloadUserFlashcards error:', err)
    return { success: false, error: 'Failed to download flashcards' }
  }
}
