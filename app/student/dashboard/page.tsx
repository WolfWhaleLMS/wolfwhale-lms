...
export default async function StudentDashboardPage() {
  ...
  if (tenantId) {
    ...
    // Fetch assigned textbooks with reading progress
    try {
      const textbookAssignments = await getMyTextbooks()
      const textbookIds = textbookAssignments.map((a: any) => a.textbook_id)

      // Fetch reading progress for all assigned textbooks
      ...
      if (textbookIds.length > 0) {
        ...
        // Count chapters per textbook
        for (const ch of allChapters || []) {
          if (!readingProgressMap[ch.textbook_id]) {
            readingProgressMap[ch.textbook_id] = { completed: 0, total: 0 }
          }
          readingProgressMap[ch.textbook_id].total++
        }

        // Count completed chapters
        for (const rp of readingProgress || []) {
          if (rp.status === 'completed') {
            if (readingProgressMap[rp.textbook_id]) {
              readingProgressMap[rp.textbook_id].completed++
            }
          }
        }
      }

      assignedTextbooks = textbookAssignments.slice(0, 4).map((a: any) => {
        const tb = a.textbook
        const progress = readingProgressMap[a.textbook_id]
        ...
        return {
          id: a.id,
          title: tb?.title || 'Textbook',
          subject: tb?.subject || 'general',
          coverImageUrl: tb?.cover_image_url || null,
          readingProgress: progressPct,
          textbookId: a.textbook_id,
        }
      })
    } catch {
