...
| Calendar/attendance/messages | Partial | Role read models, visible message composer forms, audited `/api/lms/messages` writes, audited `/api/lms/calendar-events` school/course event writes, and admin/teacher message CSV export now exist. Student/guardian messages resolve to course staff server-side, teacher messages are scoped to assigned course students/guardians, and relationship-aware calendar/message policy migrations are pending live application. Moderation, policy controls, and live RLS proof remain open. |
...
- `npm test`: passed, 34 files / 149 tests after the staff message export slice.
- `npm run lint`, `npm run typecheck`, `npm run build`, `npm audit --audit-level=moderate`, and `git diff --check`: passed after the staff message export slice.
...
- `npm run build`: passed after the staff message export slice; route list includes `/api/lms/calendar-events`, `/api/lms/courses`, `/api/lms/exports/messages`, `/api/lms/guardian-links`, `/api/lms/guardian-links/contact`, `/api/lms/guardian-links/unlink`, `/api/lms/invitations`, `/api/lms/invitations/resend`, `/api/lms/memberships/role`, `/api/lms/memberships/status`, `/api/lms/messages`, `/api/lms/submissions/[submissionId]/file`, and `/student/companion-world`.
