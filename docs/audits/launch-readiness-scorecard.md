...
| Admin setup flows | Partial | Local mutating smoke passed for course creation, enrollment, and roster import. Course setup now persists school-facing section and term labels through the admin form, LMS mutation, query mapping, exports, and `20260510225408_course_section_metadata.sql`. Direct audited admin invite, membership deactivate/reactivate, membership role-change, guardian-link, and guardian-unlink forms/routes/services now create Supabase invitations, update tenant memberships, and activate/deactivate `student_parents` relationships. Live RLS/security proof, invite resend, and deployed smoke pending. |
...
- `npm test`: passed, 32 files / 140 tests after the audited membership-role slice.
- `npm run lint`: passed after the audited membership-role slice.
- `npm run typecheck`: passed after the audited membership-role slice.
- `npm run build`: passed after the audited membership-role slice; route list includes `/api/lms/calendar-events`, `/api/lms/courses`, `/api/lms/guardian-links`, `/api/lms/guardian-links/unlink`, `/api/lms/invitations`, `/api/lms/memberships/role`, `/api/lms/memberships/status`, `/api/lms/messages`, `/api/lms/submissions/[submissionId]/file`, and `/student/companion-world`.
- `npm run load:smoke`: passed after the audited membership-role slice in 2921ms for 5000 students, 500 teachers, 1000 courses, and 50000 enrollments.
