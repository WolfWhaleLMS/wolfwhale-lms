...
| 3. Admin creates school/course/section and invites users | `app/api/lms/courses/route.ts`, `app/api/lms/enrollments/route.ts`, `app/api/lms/invitations/route.ts`, `app/api/lms/roster/import/route.ts`, `components/lms/AdminDashboard.tsx`, `lib/lms/mutations.ts`, `lib/lms/invitations.ts`, `lib/lms/queries.ts`, `lib/lms/exports.ts`, `supabase/migrations/20260510225408_course_section_metadata.sql`, `tests/lms-course-sections.test.ts`, `tests/lms-invitations.test.ts` | Partial: course section/term metadata and direct audited user invitations now exist; school lifecycle, guardian linking, invite resend/deactivation lifecycle, and live RLS/deployed proof still incomplete |
