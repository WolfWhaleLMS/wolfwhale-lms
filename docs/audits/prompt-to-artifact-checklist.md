...
| 7. Parent sees only linked child feedback/progress | `lib/lms/read-model.ts`, `lib/lms/guardian-links.ts`, `app/api/lms/guardian-links/route.ts`, `tests/lms-read-model.test.ts`, `tests/lms-guardian-links.test.ts`, `tests/pilot-data.test.ts`, guardian dashboard | Partial: local read model filters linked-child data and over-fetched private messages, and admins can now create durable parent-child links; live wrong-child/wrong-tenant RLS matrix pending |
...
| 11. Messaging works between allowed roles | `app/api/lms/messages/route.ts`, `lib/lms/mutations.ts`, `components/lms/SharedLmsPanels.tsx`, `components/lms/student-workspaces/calendar-resources-messages.tsx`, `supabase/migrations/20260510233000_course_message_write_policy.sql`, `tests/lms-messages.test.ts` | Partial: audited write workflow and relationship-aware policy artifact exist; moderation, retention/export controls, and live RLS validation remain pending |
...
| 13. File uploads are private, scoped, size/type limited, auditable | `lib/lms/mutations.ts`, `lib/lms/resource-security.ts`, `app/api/lms/submissions/[submissionId]/file/route.ts`, `components/lms/student-workspaces/shared.tsx`, `components/lms/GuardianDashboard.tsx`, storage migrations, `tests/lms-submission-file-route.test.ts`, `tests/lms-student-workspaces.test.tsx` | Partial: upload validation, signed routes, teacher/student/guardian download affordances, and audit metadata exist; live DB/storage policy validation pending |
